package exercises;
import java.util.Scanner;

public class Alice {
    public static void main(String[] args) {
        Scanner input = new Scanner(System.in);
        String alice = "Alice was beginning to get very tired of sitting by her sister on the bank, and of having nothing to do: once or twice she had peeped into the book her sister was reading, but it had no pictures or conversations in it, 'and what is the use of a book,' thought Alice 'without pictures or conversation?'";
        System.out.println("What word are you searching for?");
        String searchTerm = input.nextLine();
        if (alice.toLowerCase().contains(searchTerm.toLowerCase())) {
            Integer index = alice.indexOf(searchTerm);
            Integer length = searchTerm.length();
            System.out.println("Your search term starts at index " + index + ". Your term is " + length + " characters long.");
            String newAlice = alice.toLowerCase().replace(searchTerm.toLowerCase(), "");
            System.out.println(newAlice);
        } else {
            System.out.println("not found");
        }

    }
}
